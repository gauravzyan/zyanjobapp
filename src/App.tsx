...
const Icons = {
  ...
  Globe: ({ size = 18, color = 'currentColor' }: { size?: number, color?: string }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  MoreHorizontal: ({ size = 16, color = 'currentColor' }: { size?: number, color?: string }) => (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
const getCountryFlag = (countryName: string) => {
  ...
  if (lower.includes('united kingdom') || lower.includes('uk') || lower.includes('london') || lower.includes('england')) return '🇬🇧';
...
export default function App() {
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    if (searchKeyword.trim() !== '') {
      ...
      const companyMatch = (job.company_name || job.company)?.toLowerCase().includes(keyword);
      const descriptionMatch = job.description?.toLowerCase().includes(keyword);
      const skillsMatch = job.skills_required?.toLowerCase().includes(keyword);
      if (!titleMatch && !companyMatch && !descriptionMatch && !skillsMatch) return false;
    }

    // 2. Location/City search filter
    if (searchLocation.trim() !== '') {
      const loc = searchLocation.toLowerCase();
      const cityMatch = job.city?.toLowerCase().includes(loc);
      const countryMatch = job.country_name?.toLowerCase().includes(loc);
      const locationMatch = job.location?.toLowerCase().includes(loc);
      if (!cityMatch && !countryMatch && !locationMatch) return false;
    }
          ...
          {screen === 'auth' && (
            <div className="auth-view fade-in">
              <div className="auth-container-premium">
                <div className="indeed-logo-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '20px' }}>
                  <img src="/logo.png" alt="Logo" style={{ height: '68px', width: 'auto', objectFit: 'contain', background: '#ffffff', padding: '8px 16px', borderRadius: '16px', boxShadow: '0 6px 20px rgba(0,0,0,0.12)' }} />
                </div>
                ...
                <form onSubmit={handleAuthSubmit} className="mobile-form-premium">
                  {authMode === 'register' && (
                    <div className="mob-input-wrap-premium" style={{ opacity: otpRequired ? 0.6 : 1 }}>
                  ...
                  {otpRequired && (
                    <div className="mob-input-wrap-premium animate-slide-up">
                      <label className="input-label-premium" style={{ color: '#10b981', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        <Icons.Sparkles size={12} color="#10b981" /> Verification Code (OTP) *
                      </label>
                      <input 
                        type="text" 
                        required 
                ...
                <div className="auth-toggle-premium">
                  {authMode === 'login' ? (
                    <span>
                      New to Zyan?{' '}
                      <button onClick={() => { setAuthMode('register'); setOtpRequired(false); setOtp(''); setPhone(''); setErrorMessage(null); setSuccessMessage(null); }} className="auth-toggle-btn-premium">
                        Register Profile
                      </button>
                    </span>
                  ) : (
                    <span>
                      ...
                      <button onClick={() => { setAuthMode('login'); setOtpRequired(false); setOtp(''); setPhone(''); setErrorMessage(null); setSuccessMessage(null); }} className="auth-toggle-btn-premium">
                        Sign in
                      </button>
                    </span>
                  )}
              ...
              {selectedJob ? (
                ...
                <>
                  ...
                  <div className="indeed-header">
                    <div style={{ display: 'flex', alignItems: 'center' }}>
                      <img src="/logo.png" alt="Logo" style={{ height: '50px', width: 'auto', objectFit: 'contain', background: '#ffffff', padding: '4px 12px', borderRadius: '12px', boxShadow: '0 3px 10px rgba(0,0,0,0.06)' }} />
                    </div>
                    <div className="header-right-icons">
                      <div className="header-circle-btn" onClick={() => setShowNotificationsSheet(true)}>
                        <Icons.Bell size={18} color="#0f172a" />
                        {notifications.length > 0 && <span className="bell-icon-dot" />}
                      </div>
                      <div className="header-circle-btn" onClick={() => setActiveTab('profile')}>
                        <Icons.User size={18} color="#0f172a" />
                      </div>
                    </div>
                  </div>
                ...
                {activeTab === 'home' && (
                  <div className="tab-view fade-in">
                    ...
                    {/* Floating 2-Row Search Block Card */}
                    <div className="indeed-search-block">
                      <div className="search-row border-bottom">
                        <span className="search-icon-svg"><Icons.Search size={18} color="#2563eb" /></span>
                        <input 
                          type="text" 
                        ...
                        />
                        <span className="search-right-icon"><Icons.Globe size={18} color="#2563eb" /></span>
                      </div>
                      <div className="search-row">
                        <span className="search-icon-svg"><Icons.MapPin size={18} color="#2563eb" /></span>
                        <input 
                          type="text" 
                        ...
                        />
                        <span className="search-right-icon"><Icons.Target size={18} color="#2563eb" /></span>
                      </div>
                    </div>
                    ...
                    <div className="jobs-section-header">
                      <div>
                        ...
                        <p className="section-subtitle-light">Curated roles for international candidates</p>
                      </div>
                    ...
                    <div className="job-cards-feed">
                      {loadingJobs ? (
                        ...
                        <div className="empty-state-card" style={{ padding: '24px', background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '24px', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                          <Icons.Folder size={36} color="#9ca3af" />
                          <p style={{ fontSize: '13px', color: '#64748b', margin: '4px 0 0 0' }}>No relocation openings match the current filters.</p>
                        </div>
                      ) : (
                        filteredJobs.map((job) => {
                          const countryFlag = getCountryFlag(job.country_name || job.city);
                          const isSaved = savedJobIds.includes(job.id);
                          const respondsText = job.responds || (job.id % 2 === 0 ? 'Replies in 24h' : 'Replies in 1 day');

                          return (
                            <div key={job.id} className="indeed-glass-card cursor-pointer" onClick={() => setSelectedJob(job)}>
                              
                              {/* Top Row: Badge & Bookmark/More Actions */}
                              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                {job.is_featured === 1 ? (
                                  <span className="urgently-badge">FEATURED</span>
                                ) : (
                                  <div />
                                )}

                                <div className="bookmark-dislike-col" style={{ flexDirection: 'row' }}>
                                  <button onClick={(e) => { e.stopPropagation(); toggleSave(job.id); }} className="action-circle-btn">
                                    <Icons.Bookmark size={15} color={isSaved ? "#2563eb" : "#64748b"} filled={isSaved} />
                                  </button>
                                  <button className="action-circle-btn" onClick={(e) => e.stopPropagation()}>
                                    <Icons.MoreHorizontal size={15} color="#64748b" />
                                  </button>
                                </div>
                              </div>
                              ...
                              <div className="card-header-row">
                                <div className="company-logo-box" onClick={(e) => { e.stopPropagation(); fetchCompanyDetails(job.company_slug || 'tokyo-tech-labs'); }} style={{ background: job.company_name?.toLowerCase().includes('bluepeak') ? 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)' : 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)', cursor: 'pointer' }}>
                                  {job.company_logo && (job.company_logo.startsWith('http://') || job.company_logo.startsWith('https://')) ? (
                                    <img src={job.company_logo} alt="Logo" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                  ) : (
                                ...
                                <div className="card-title-block">
                                  ...
                                  <div className="company-rating-row">
                                    <span className="company-name-text" onClick={(e) => { e.stopPropagation(); fetchCompanyDetails(job.company_slug || 'tokyo-tech-labs'); }} style={{ cursor: 'pointer' }}>
                                      {job.company_name || job.company}
                                    </span>
                                    <span className="rating-star-badge">★ {job.rating || '4.6'}</span>
                                  </div>
                                  
                                  <p className="location-text">
                                    <Icons.MapPin size={13} color="#64748b" style={{ flexShrink: 0 }} />
                                    <span>{job.city}, {job.country_name} {countryFlag}</span>
                                  </p>
                                </div>

                                {/* Speed Bolt Badge */}
                                <div className="responds-speed-badge">
                                  <Icons.Bolt size={12} color="#059669" /> {respondsText}
                                </div>
                              </div>

                              {/* Color-Coded Perks Tags */}
                              <div className="perks-pills-wrap">
                                <span className="success-perk-pill">💼 {job.job_type || 'Full-time'}</span>
                                <span className="visa-perk-pill">🛡️ Visa Sponsored</span>
                                <span className="relocation-perk-pill">✈️ Relocation Support</span>
                              </div>

                              {/* Bottom Salary Info */}
                              <div className="salary-bottom-row">
                                <span className="salary-amount-text">
                                  💵 {job.salary_min ? `${job.currency === 'GBP' ? '£' : job.currency === 'EUR' ? '€' : '$'}${Number(job.salary_min).toLocaleString()} – ${job.currency === 'GBP' ? '£' : job.currency === 'EUR' ? '€' : '$'}${Number(job.salary_max).toLocaleString()} ${job.currency || 'GBP'} / year` : job.salary || 'Competitive Salary'}
                                </span>
                              </div>
              ...
              <div className="indeed-bottom-tabs-nav">
                ...
                >
                  <span className="tab-icon-box">
                    <Icons.Home color={activeTab === 'home' ? '#2563eb' : '#64748b'} size={20} />
                  </span>
                  <span className="nav-lbl-text" style={{ fontSize: '11px', marginTop: '2px' }}>Home</span>
                </button>
                ...
                >
                  <span className="tab-icon-box">
                    <Icons.Bookmark color={activeTab === 'myjobs' ? '#2563eb' : '#64748b'} size={20} filled={activeTab === 'myjobs'} />
                  </span>
                  <span className="nav-lbl-text" style={{ fontSize: '11px', marginTop: '2px' }}>Saved Jobs</span>
                </button>
                ...
                >
                  <span className="tab-icon-box">
                    <Icons.Messages color={activeTab === 'messages' ? '#2563eb' : '#64748b'} size={20} />
                  </span>
                  <span className="nav-lbl-text" style={{ fontSize: '11px', marginTop: '2px' }}>Messages</span>
                </button>
                ...
                >
                  <span className="tab-icon-box">
                    <Icons.User color={activeTab === 'profile' ? '#2563eb' : '#64748b'} size={20} />
                  </span>
                  <span className="nav-lbl-text" style={{ fontSize: '11px', marginTop: '2px' }}>Profile</span>
                </button>
              </div>
            </>
            )}
